    ...
    <body>
        ...
        <table border="1">
            ...
            <tbody>
                <% if (newsList != null) {
                    ...
                    <tr>
                        <td><%= number %></td>
                        <td><a href="content.do?num=<%= article.getNum() %>"><%= article.getTitle() %></a></td>
                        <td><%= article.getId() %></td>
        ...
        <div align="center">
            <% if (newsList != null && newsList.size() > 0) {
                ...
                if (startPage > 1) { %>
                    <a href="newslist.do?pageNum=<%= startPage - 1 %>&newsType=<%= newsType %>">[이전]</a>
                <% }
                ...
                for (int i = startPage; i <= endPage; i++) { %>
                    <% if (i == currentPage) { %>
                        <%= i %>
                    <% } else { %>
                        <a href="newslist.do?pageNum=<%= i %>&newsType=<%= newsType %>"><%= i %></a>
                    <% } %>
                <% }

                if (endPage < pageCount) { %>
                    <a href="newslist.do?pageNum=<%= endPage + 1 %>&newsType=<%= newsType %>">[다음]</a>
                <% }
            } %>
        </div>
    </body>
